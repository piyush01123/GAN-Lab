...
f = open(matplotlib.matplotlib_fname(),'w')
f.write('backend : Agg')
f.close()
matplotlib.use('Agg')
...
class DCGAN:
    def __init__(self, num_steps, save_dir, save_interval, batch_size):
    ...
    def plot_images(self, step, num_images = 16):
        ...
        utils.copy_to_gcs('step.png', '%s/%s.png' %(self.save_dir, step))
    ...
    def save_model(self):
        ...
        utils.copy_to_gcs('discriminator.h5', '%s/%s' %(self.save_dir, 'discriminator.h5'))
        utils.copy_to_gcs('generator.h5', '%s/%s' %(self.save_dir, 'generator.h5'))
        utils.copy_to_gcs('adversarial.h5', '%s/%s' %(self.save_dir, 'adversarial.h5'))
